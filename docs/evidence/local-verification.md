...
| Public signed-out Chrome preview | 13 checks passed after the font-preload correction; no captured page errors or console warnings. |
| GitHub Actions | The first two public commits completed successfully; run IDs 35770940973 and 35771784159. The docs-only final commit needs its own CI result. |
...
Public sample URL: https://marginfield-reader.soumikhalder.chatgpt.site. This is a functional UI preview only. The first public version exposed two unused-font-preload warnings; the second removed those links and the signed-out rerun captured zero warnings.
